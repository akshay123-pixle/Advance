...
let bands: string[] = []; // explicitly string array
...
let myTuple: [string, number] = ["akshay", 90]; // ✅ OK - fixed structure: [string, number]
...
let myObj: object; // object type - can be any non-primitive (array, object, etc.)
...
// exampleObj.prop2 = 42; // ❌ Error - Type 'number' not assignable to 'boolean'
...
type Guitarish = {
  name: string;
  active: boolean;
  albums: (boolean | string | number)[]; // union type in array
};
